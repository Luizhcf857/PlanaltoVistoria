...
public class DashBoardCliente extends JPanel {
    ...
    private JPanel criarPainelAgendamentos() {
        ...
        tableAgendamentos.getColumn("Ação")
        .setCellEditor(new ButtonEditor(
            new JCheckBox(), 
            painelPrincipal, 
            cardPrincipal, 
            tableAgendamentos, 
            clienteAtual // variável que contém o cliente atual logado
        ));
    ...
    private void carregarAgendamentosClienteComPagamento() {
        ...
        for (Agendamento ag : agendamentos) {
            ...
            // Se não houver vistoria, adiciona linha com status "Sem vistoria"
            if (vistorias.isEmpty()) {
                ...
                modelTabela.addRow(new Object[]{
                        ...
                        ag.getTipoServico(),
                        "Sem vistoria",
                        null, // idVistoria vazio
                        "Pagar"
                });
            } else {
                // Se houver vistorias, adiciona normalmente
    ...
    class ButtonEditor extends DefaultCellEditor {
        ...
        public Object getCellEditorValue() {
            if (isPushed) {
                ...
                // Cria tela de pagamento
                PagamentoView telaPagamento = new PagamentoView(painelPrincipal, cardPrincipal, pagamento);

                // Adiciona tela de pagamento ao painel principal (CardLayout do BaseTelas)
                painelPrincipal.add(telaPagamento, "telaPagamento");

                // Mostra a tela de pagamento
                cardPrincipal.show(painelPrincipal, "telaPagamento");

                // 🔧 Força o redesenho da janela principal (corrige o bug de não aparecer)
                java.awt.Window janela = SwingUtilities.getWindowAncestor(painelPrincipal);
                if (janela != null) {
                    janela.revalidate();
                    janela.repaint();
                } else {
